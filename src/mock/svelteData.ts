export const svelteTemplates: any = [
	{
		filename: 'src',
		value: '',
		path: 'src',
		...
		children: [
			{
				filename: 'index.html',
				value: `<div class="totalHeader">This is svelte page</div>
<input type="text" bind:value="{name}" />
<p>Count:{count}</p>
<button on:click="{increaseCount}">+1</button>
<h3>Hello {@html name.split('')}</h3>
<div>
    {#each name.split('') as char, i}
    <span
        style="
            padding: 5px;
            background: rgb({i * 20 % 255}, {i * 20 % 255}, {i * 20 % 255});
            color: rgb(
                {255-i * 20 % 255},
                {255-i * 20 % 255},
                {255-i * 20 % 255}
...
<div style="margin-top: 20px">{time.toLocaleTimeString()}</div>
<label>
...
`,
			...
			{
				filename: 'index.css',
				value: `@import url('https://fonts.googleapis.com/css2?family=Roboto:wght@100&display=swap');
* {
