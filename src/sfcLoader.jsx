...
.read-the-docs {
  color: #dedede;
    ...
    let optionList = [
      { type: 'root', op: options, name: '' },
      { type: 'component', op: cmptOption, name: 'ButtonCmpt' },
    ];
    let rootApp = null;
    if (!curVueApp) {
  ...
  const compileVue = options => {
    if (!curVueApp) {
      ...
      curVueApp.unmount();
      let nextVueApp = Vue.createApp(
        Vue.defineAsyncComponent(() =>
          window['vue3-sfc-loader'].loadModule('child.vue', options)
        )
      );
