...
import { solidTemplates } from '@mock/solidData';
...
function Directory(props: any) {
	...
	// solid page path inject
	const [selectedKeys, setSelectedKeys] = useState([
		solidTemplates[0].children[0].path
	]);
	...
	// solid page template inject
	const [spendKeys, setSpendKeys] = useState([solidTemplates[0]?.path]);
	...
	const finishChangeName = (e: any) => {
		...
		updateData(
			...
			// solid page db name inject
			'solid-ng-templates',
			changedData
		);
		// 重新编译
		const currentFile = templatesAfter[0].children.find((e: any) =>
			// solid page entry inject
			e.filename.includes('index.html')
		);
		// solid page parseMethod inject
		parseSolid(currentFile?.value || '', templatesAfter);
		setShowOP('');
		setShowRename('');
		setShowRenameKey('');
	};
	const AddFile = (event: any) => {
		...
		updateData(
			(editStore as any).currentIndexDBInstance.db,
			// solid page update db inject
			'mika-solid-templates',
			changedData
		);
	};
	...
	const deleteData = (event: any, node: any) => {
		...
		Modal.confirm({
			...
			onOk: () => {
				...
				// solid page parse method inject
				parseSolid(currentFile?.value || '', templatesAfter);
				onSelect([], {
					node: currentFile
				});
	...
	useEffect(() => {
		const curData = {
			...
			storeName: 'mika-solid-templates', //当前的数据库名
			version: 1 //版本号
		};
		const info = {
			id: 'daryl',
		...
		setTimeout(async () => {
			...
			try {
				...
				indexStore?.templates
					...
					: // solid page db init inject
						addData(curRequest.db, 'mika-solid-templates', info);
				// 默认文件加载
				const currentFile = (indexStore?.templates ||
					// solid page default file init inject
					solidTemplates)[0].children.find((e: any) =>
					// solid page default file init inject
					e.filename.includes('index.html')
				);
				// 默认文件store相关存储
				editStore.updateCode(
					currentFile.value,
					// solid page update file init inject
					'index.html'
				);
				...
				const getRootDom = setInterval(() => {
					document.getElementById('previewFrame') &&
						(() => {
							...
							parseSolid(
								currentFile?.value || '',
								indexStore?.templates || solidTemplates
							);
