...
function Directory(props: any) {
	...
	// ember page path inject
	const [selectedKeys, setSelectedKeys] = useState([
		emberTemplates[0].children[0].path
	]);
	...
	// ember page template inject
	const [spendKeys, setSpendKeys] = useState([emberTemplates[0]?.path]);
	...
	const finishChangeName = (e: any) => {
		...
		// ember page parseMethod inject
		parseEmber(currentFile?.value || '', templatesAfter);
		setShowOP('');
		setShowRename('');
		setShowRenameKey('');
	};
	...
	const deleteData = (event: any, node: any) => {
		...
		Modal.confirm({
			...
			onOk: () => {
				...
				// ember page parse method inject
				parseEmber(currentFile?.value || '', templatesAfter);
				onSelect([], {
					node: currentFile
				});
	...
	useEffect(() => {
		const curData = {
			...
			storeName: 'mika-ember-templates', //当前的数据库名
			version: 1 //版本号
		};
		const info = {
			id: 'daryl',
		...
		setTimeout(async () => {
			...
			try {
				...
				indexStore?.templates
					...
					: // ember page db init inject
						addData(curRequest.db, 'mika-ember-templates', info);
				// 默认文件加载
				const currentFile = (indexStore?.templates ||
					// ember page default file init inject
					emberTemplates)[0].children.find((e: any) =>
					// ember page default file init inject
					e.filename.includes('index.html')
				);
				// 默认文件store相关存储
				editStore.updateCode(
					currentFile.value,
					// ember page update file init inject
					'index.html'
				);
				...
				const getRootDom = setInterval(() => {
					document.getElementById('previewFrame') &&
						(() => {
							...
							parseEmber(
								currentFile?.value || '',
								indexStore?.templates || emberTemplates
							);
