...
function Directory(props: any) {
	const { cpType = 'lit' } = props;
	const [defaultFile, setDefaultFile] = useState({});
	// lit page path inject
	const [selectedKeys, setSelectedKeys] = useState([
		litTemplates[0].children[0].path
	]);
	...
	// lit page template inject
	const [spendKeys, setSpendKeys] = useState([litTemplates[0]?.path]);
	...
	const finishChangeName = (e: any) => {
		...
		updateData(
			...
			// lit page db name inject
			'lit-ng-templates',
			changedData
		);
		// 重新编译
		const currentFile = templatesAfter[0].children.find((e: any) =>
			// lit page entry inject
			e.filename.includes('index.html')
		);
		// lit page parseMethod inject
		parseLit(currentFile?.value || '', templatesAfter);
		setShowOP('');
		setShowRename('');
		setShowRenameKey('');
	};
	...
	useEffect(() => {
		const curData = {
			...
			storeName: 'mika-lit-templates', //当前的数据库名
			version: 1 //版本号
		};
		const info = {
			id: 'daryl',
		...
		setTimeout(async () => {
			...
			try {
				...
				indexStore?.templates
					...
					: // lit page db init inject
						addData(curRequest.db, 'mika-lit-templates', info);
				// 默认文件加载
				const currentFile = (indexStore?.templates ||
					// lit page default file init inject
					litTemplates)[0].children.find((e: any) =>
					// lit page default file init inject
					e.filename.includes('index.html')
				);
				// 默认文件store相关存储
				editStore.updateCode(
					currentFile.value,
					// lit page update file init inject
					'index.html'
				);
				...
				const getRootDom = setInterval(() => {
					document.getElementById('previewFrame') &&
						(() => {
							...
							parseLit(
								currentFile?.value || '',
								indexStore?.templates || litTemplates
							);
