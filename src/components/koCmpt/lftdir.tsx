...
import { koTemplates } from '@mock/koData';
...
function Directory(props: any) {
	const { cpType = 'ko' } = props;
	const [defaultFile, setDefaultFile] = useState({});
	// ko page path inject
	const [selectedKeys, setSelectedKeys] = useState([
		koTemplates[0].children[0].path
	]);
	...
	// ko page template inject
	const [spendKeys, setSpendKeys] = useState([koTemplates[0]?.path]);
	...
	const finishChangeName = (e: any) => {
		...
		updateData(
			...
			// ko page db name inject
			'ko-ng-templates',
			changedData
		);
		// 重新编译
		const currentFile = templatesAfter[0].children.find((e: any) =>
			// ko page entry inject
			e.filename.includes('index.html')
		);
		// ko page parseMethod inject
		parseKnockOut(currentFile?.value || '', templatesAfter);
		setShowOP('');
		setShowRename('');
		setShowRenameKey('');
	};
	...
	useEffect(() => {
		const curData = {
			...
			storeName: 'mika-ko-templates', //当前的数据库名
			version: 1 //版本号
		};
		const info = {
			id: 'daryl',
		...
		setTimeout(async () => {
			...
			try {
				...
				indexStore?.templates
					...
					: // ko page db init inject
						addData(curRequest.db, 'mika-ko-templates', info);
				// 默认文件加载
				const currentFile = (indexStore?.templates ||
					// ko page default file init inject
					koTemplates)[0].children.find((e: any) =>
					// ko page default file init inject
					e.filename.includes('index.html')
				);
				// 默认文件store相关存储
				editStore.updateCode(
					currentFile.value,
					// ko page update file init inject
					'index.html'
				);
				...
				const getRootDom = setInterval(() => {
					document.getElementById('previewFrame') &&
						(() => {
							...
							parseKnockOut(
								currentFile?.value || '',
								indexStore?.templates || koTemplates
							);
