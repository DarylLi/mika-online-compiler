...
import { pugTemplateTemplates } from '@mock/pugTemplateData';
...
function Directory(props: any) {
	...
	// pug page path inject
	const [selectedKeys, setSelectedKeys] = useState([
		pugTemplateTemplates[0].children[0].path
	]);
	...
	// pug page template inject
	const [spendKeys, setSpendKeys] = useState([pugTemplateTemplates[0]?.path]);
	...
	const finishChangeName = (e: any) => {
		...
		const currentFile = templatesAfter[0].children.find((e: any) =>
			...
			e.filename.includes('index.pug')
		);
		// pug page parseMethod inject
		parsePugtemplate(currentFile?.value || '', templatesAfter);
		setShowOP('');
		setShowRename('');
		setShowRenameKey('');
	};
	...
	useEffect(() => {
		const curData = {
			db: null,
			// pug page db init inject
			storeName: 'mika-pug-templates', //当前的数据库名
			version: 1 //版本号
		};
		const info = {
			id: 'daryl',
		...
		setTimeout(async () => {
			...
			try {
				...
				indexStore?.templates
					...
					: // pug page db init inject
						addData(curRequest.db, 'mika-pug-templates', info);
				// 默认文件加载
				const currentFile = (indexStore?.templates ||
					// pug page default file init inject
					pugTemplateTemplates)[0].children.find((e: any) =>
					// pug page default file init inject
					e.filename.includes('index.pug')
				);
				// 默认文件store相关存储
				editStore.updateCode(
					currentFile.value,
					// pug page update file init inject
					'index.pug'
				);
				...
				const getRootDom = setInterval(() => {
					document.getElementById('previewFrame') &&
						(() => {
							...
							parsePugtemplate(
								currentFile?.value || '',
								indexStore?.templates || pugTemplateTemplates
							);
					...
					titleRender={(node: any) =>
						showRenameKey === node.path ? (
							...
							<>
								...
								{showOP === node.path &&
									!(
										/App.vue|app.jsx|index.pug|index.js|index.css/.test(
											showOP
										) || showOP === 'src'
									) && (
